    ...
    public static BufferedImage embed(BufferedImage originalImage, String text)
    {
        ...
        for (int i = 0; i <= text.length(); i++)
        {
            ...
            for (int j = 0; j < 8; j++)//8 bits forms a character.
            {
                ...
                if (bitValue == 1)
                {
                    originalImage.setRGB(x, y, originalImage.getRGB(x, y) | EXTRACTOR);   //Replaces least significant value of the
                                                                                              // blue color of the pixel with 1.
                } else
                {
                    originalImage.setRGB(x, y, originalImage.getRGB(x, y) & ZEROATLAST);  //Replaces least significant value of the
                                                                                              // blue color of the pixel with 0.
                }
                x++;
